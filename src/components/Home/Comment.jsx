  ...
  const comments = [
    {
      ...
      deskripsi:
        "Lorem ipsum dolor sit, amet consectetur adipisicing elit. Dolorum, possimus.",
    },
    {
      id: 2,
      images: Avatar,
      ...
      <section className="section1 flex flex-col justify-center font-poppins items-center py-[70px] gap-y-[70px] max-w-[70rem] mx-auto">
        ...
        <div className="grid grid-cols-1 tablet:grid-cols-2 laptop:grid-cols-3 gap-x-8 gap-y-10">
          {comments.map((data) => (
            <div
              key={data.id}
              className="flex flex-col justify-start gap-3 p-4 bg-white shadow-xl rounded-2xl"
            >
              <img
                src={data.images}
                alt={data.nama}
              ...
              <div className="text-[14px] flex flex-col gap-1">
                ...
                <small className="text-slate-400">{data.jabatan}</small>
              </div>
            </div>
          ))}
        </div>
